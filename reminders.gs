...
function woodlandReminder() {
  admin = Browser.msgBox("Test Email", Browser.Buttons.YES_NO);
  var date = Browser.inputBox("Enter Date of Meeting:", Browser.Buttons.OK);
  ...
  for (i in pData) {
    var row = pData[i];
    if (Utilities.formatDate(row[0], "GMT+0", "MM/d/yyyy") == date) {
      var sheetRow = parseInt(i)+4;
      //Foxes
      BuildEmail(row, 5, 6, "Fox", date, sheetRow);
      //Hawks 1
      BuildEmail(row, 7, 8, "Hawks (A-J)", date, sheetRow);
      //Hawks 2
      BuildEmail(row, 9, 10, "Hawks (K-Z)", date, sheetRow);
      //Mt. Lions 1
      BuildEmail(row, 11, 12, "Mt. Lions (A-J)", date, sheetRow);
      //Mt. Lions 2
      BuildEmail(row, 13, 14, "Mt. Lions (K-Z)", date, sheetRow);
    }
  }
  
}

function BuildEmail(row, teachIndex, assistantIndex, patrol, date, rowNum) {
  ...
  var subject = 'Teaching Reminder for the ' + patrol + ' patrol on ' + date;
